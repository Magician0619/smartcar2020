...
Img_id = multiprocessing.Value("i",0)#speed and angle (int)

img_id_file = open("img.value",'r')
Img_id.value = 0
img_id_file.close()
print("id starts with:",Img_id.value)
...
def save_image_process(lock,n,status,start,Camera,data,img_id):
    ...
    while status.value:#PS2 tr or tl control stop
        ...
        value = int(img_id.value)
        cv2.imwrite(path+"/data/"+save_name+"/{}.jpg".format(value), frame)
        #a.value = imgInd
        file_write.write(str(data[1]))
        file_write.write("\n")
        file_write.flush()
        print("imgInd=",img_id.value)
...
if __name__ == '__main__':
    ...
    try:
        ...
        process_image = multiprocessing.Process(target=save_image_process,args=(lock,Flag_save_data,Status,START,camera,Speed,Img_id))
        ...
        RUN.value = False
        print("finally")
        img_id_file = open("img.value",'w')
        img_id_file.write(str(Img_id.value))
        img_id_file.close()
