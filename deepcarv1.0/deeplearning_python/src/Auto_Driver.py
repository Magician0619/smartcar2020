...
#    return img
out = cv2.VideoWriter('ad_save.avi',cv2.VideoWriter_fourcc(*'MJPG'),20,(320,240))
...
def dataset(video):
    ...
    mask9 = np.array([mask for i in range(3)]).transpose(1,2,0)
    
    print("mask shape",mask9.shape)
    out.write(mask9)
    ...
    try:
        ...
        while 1:
            while 1:
                ...
                a = int(angle*1600 + 700)
                #input("angle send:"+str(a))
                lib.send_cmd(vel, a)
                print(cout)
                cout=cout+1
