    ...
    while 1:
        ...
        while 1:
            ...
            if len(bboxes.shape) == 1 :
                ...
                STATE_value = False
                # labels in (no_limit(0),limit(1),park(2),red(3),green(4),block(5))
            ...
            true_angle = int(angle*2000 + 500)

            print("sending: angle: %d, label: %d" % (true_angle, int(best_label)))

            # user_cmd(STATE_value,t_labels,t_scores,center_x,center_y,vel,a)

            lib.send_cmd(true_angle, int(best_label))
            print(cout)
            cout=cout+1
